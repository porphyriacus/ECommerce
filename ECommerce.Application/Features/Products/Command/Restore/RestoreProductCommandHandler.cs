...
namespace ECommerce.Application.Features.Products.Command.Restore
{
    ...
    internal class RestoreProductCommandHandler(IUnitOfWork unitOfWork, IMediator mediator)
        : IRequestHandler<RestoreProductCommand, Result>
    {
        public async Task<Result> Handle(RestoreProductCommand request, CancellationToken cancellation)
        {
            ...
            try
            {
                ...
                if (product == null)
                    return Error.NotFound("RestoreProductCommand", $"Product with id {request.productId} nit found");
                await unitOfWork.Products.RestoreAsync(product, cancellation);
            ...
            catch (Exception ex)
            {
                ...
                return Result.Failure(Error.Failure("RestoreProductCommand", ex.Message));
            }
        }
    }
}
